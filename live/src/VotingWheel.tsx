...
export default function VotingWheel() {
  ...
  const animRef = useRef<ReturnType<typeof animateValue> | null>(null);
  ...
  const spinWheel = () => {
    ...
    animRef.current = animateValue(rotation, targetAngle, {
      ...
      onComplete: () => {
        setIsSpinning(false);
        update(ref(db, 'session'), { currentAngle: targetAngle }).catch(console.error);
        set(ref(db, 'session/spin'), { active: false, startAngle, targetAngle, durationMs, startedAt: 0 }).catch(console.error);
  ...
  const stopWheel = () => {
    ...
    update(ref(db, 'session'), { currentAngle: finalAngle }).catch(console.error);
    set(ref(db, 'session/spin'), { active: false, startAngle: finalAngle, targetAngle: finalAngle, durationMs: 0, startedAt: 0 }).catch(console.error);
    determineWinner(optionsRef.current);
  };
